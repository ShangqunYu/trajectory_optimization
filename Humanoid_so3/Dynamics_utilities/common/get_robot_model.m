...
%
% ************     Planar Quadruped Model using 3D spatial notation:           ************
...
% 5 - Rear Foot
%
% ************      Planar Humanoid Model using 3D spatial notation:           ************
% Coordinate system: x forward, y left, z up
% Bodies:
% 0   - fixed origin
% 1   - Px (translate horizontal) (massless)
...
if(strcmp(params.model,'quad')) % 2D quadruped model in spatial coords
    ...
    %% Quadruped 2D
elseif(strcmp(params.model,'quad2D')) % 2D quadruped model in 2D spatial coords
    % Various mass parameters and locations
    Ipitch = 1;
    I_body = mcI(params.body_mass, [0 0], params.i_body_mult * Ipitch);
    I_l1   = mcI(params.l1_mass, [0 -params.l1/2], .1);
    I_l2   = mcI(params.l2_mass, [0 -params.l2/2], .1);
    hip_x = [params.body_length -params.body_length]/2;
    NLEGS = 2;
    ...
    model.Xfoot   = repmat({zeros(3)},NLEGS,1);    % feet
    model.b_foot  = zeros(1,NLEGS);
    nb = 0; % current body index
    ...
    model.parent(nb) = nb - 1;   % parent is previous
    model.jtype{nb}  = 'px';     % prismatic x
    model.Xtree{nb}  = eye(3);   % on top of previous joint
    model.I{nb}      = zeros(3); % massless
    % Base z translation (no mass)
    nb = nb + 1;
    model.parent(nb) = nb - 1;
    ...
    for leg = 1:NLEGS
        ...
        model.parent(nb) = nb_base;  % hip parent is base
        model.jtype{nb}  = 'r';     % revolute
        model.Xtree{nb}  = plnr(0, [hip_x(leg) 0]);
        model.I{nb}      = I_l1;
        % Knee
        nb = nb + 1;
        model.parent(nb) = nb - 1; % knee parent is hip
        model.jtype{nb}  = 'r'; % revolute
        model.Xtree{nb}  = plnr(0,[0 -params.l1]');
        model.I{nb}      = I_l2;
        % Foot (bonus)
        model.Xfoot{leg}  = plnr(0,[0 -params.l2]');
        model.b_foot(leg) = nb;
    end
    disp(['Created robot model with ' num2str(nb) ' coordinates!']);
...
elseif(strcmp(params.model,'quad3D')) % 3D quadruped model in spatial coords
    ...
    for leg = 1:model.NLEGS
        ...
        nb = nb + 1;
        model.parent(nb) = nb_base;  % hip parent is base
        model.jtype{nb}  = 'Rx';     % rotate around x
...
elseif(strcmp(params.model,'humanoid3D')) % 3D humanoid model
    ...
    model.b_hand    = zeros(1,NARMS);
    q_arm = [0 0 -0.1];
    q_leg = [0 0 deg2rad(-45) deg2rad(60) -0.67];
    ...
    model.indx.arm = sort([model.indx.shoulderx model.indx.shouldery model.indx.elbow]);
    ...
    %% Humanoid 3D (no arms)
elseif(strcmp(params.model,'humanoidNoArms3D')) % 3D humanoid model
    % Various mass parameters and locations
    I_body = mcI(params.torso_mass, params.torso_COM, boxInertia(params.torso_mass,[params.torso_length, params.torso_width, params.torso_height]));
    I_hipRz = mcI(params.hipRzMass,params.hipRzCOM,reshape(params.hipRzRotationalInertia,3,3));
    I_hipRx = mcI(params.hipRxMass,params.hipRxCOM,reshape(params.hipRxRotationalInertia,3,3));
    ...
    model.q_home = [zeros(1,6) repmat(q_leg,1,NLEGS)];
